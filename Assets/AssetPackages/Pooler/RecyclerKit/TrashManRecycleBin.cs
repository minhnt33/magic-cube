    ...
    public GameObject spawn()
    {
        ...
        if (go != null)
        {
            ...
            if (automaticallyRecycleParticleSystems)
            {
                ...
                // Find maximum life time of particle system
                float maxLifeTime = systems[0].duration + systems[0].startLifetime;
                for (int i = 1; i < systems.Length - 1; i++)
                {
                    maxLifeTime = Mathf.Max(maxLifeTime, systems[i].duration + systems[i].startLifetime);
                }
                ...
                TrashMan.despawnAfterDelay(go, maxLifeTime);
            }
        }
