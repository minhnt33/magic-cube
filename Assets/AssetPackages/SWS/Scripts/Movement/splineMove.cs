...
namespace SWS
{
    ...
    public class splineMove : MonoBehaviour
    {
        ...
        public bool _lookAtPoint;
        public Vector3 _lookAtTarget;
        ...
        // Easy way to get next point index
        public int NextPoint { get { return Mathf.Clamp(currentPoint + 1, 0, waypoints.Length - 1); } }
        ...
        private void CreateTween()
        {
            ...
            else
            {
                if (local)
                {
                    tween = transform.DOLocalPath(wpPos, originSpeed, pathType, pathMode)
                                    .SetAs(parms)
                                    .SetOptions(closeLoop, lockPosition, lockRotation).SetLookAt(lookAhead);
                }
                else
                {
                    tween = transform.DOPath(wpPos, originSpeed, pathType, pathMode)
        ...
        /// <summary>
        /// Rotate to face agent to a gameobject
        /// </summary>
        /// <param name="target"></param>
        /// <param name="sec"></param>
        /// <param name="stop"></param>
        public void FaceToPoint(Transform target, float sec, float stop, bool resume = false)
        {
            StartCoroutine(LookAtCor(target, sec, stop, resume));
        }

        private IEnumerator LookAtCor(Transform target, float sec, float stop, bool resume)
        {
            float timer = sec;
            float stopTimer = 0f;
            float progress = 0f;
            ...
            while (true)
            {
                Vector3 dir = target.position - agentTrans.position;
                ...
                agentTrans.rotation = Quaternion.Slerp(agentTrans.rotation, targetQuat, progress);
