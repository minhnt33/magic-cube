...
namespace SWS
{
    ...
    public class navMove : MonoBehaviour
    {
        ...
        public void FaceToPoint(Transform target, float sec, float stop)
        {
            StartCoroutine(LookAtCor(target, sec, stop));
        }

        private IEnumerator LookAtCor(Transform target, float sec, float stop)
        {
            float timer = sec;
            float stopTimer = 0f;
            float progress = 0f;
